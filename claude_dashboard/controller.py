...
"""Main application controller — session discovery + streaming state detection."""
...
from claude_dashboard.transcript import StatusState, TranscriptWatcher
...
class _SessionEntry:
    """Tracks a live session: its info, watcher, container, and current state."""

    __slots__ = ("session", "watcher", "container", "state")
...
class AppController:
    """Central controller — discovers sessions, streams transcripts, updates UI."""
    ...
    def __init__(self, *, debug: bool = False, quiet: bool = False, ttl_seconds: int = 0):
        ...
        # PID -> _SessionEntry (live sessions with their watchers)
    ...
    def run(self):
        ...
        # Discovery tick — finds new/dead sessions, watchers handle state
    ...
    def _discovery_tick(self):
        """Discover new sessions and remove dead ones. State is handled by watchers."""
        try:
    ...
    def _add_session(self, session: SessionInfo):
        ...
        if transcript_path:
            ...
            def make_callback(pid: int = session.pid, project: str = cwd_short):
                def on_change(new_state: StatusState):
                    # Schedule UI update on main thread
                    self._root.after(0, self._on_state_change, pid, project, new_state)

                return on_change

            entry.watcher = TranscriptWatcher(
                transcript_path,
                on_state_change=make_callback(),
            )
            entry.watcher.start()
        ...
        logger.debug(
            ...
            session.pid,
            session.cwd.rsplit("\\", 1)[-1].rsplit("/", 1)[-1],
            entry.state.value,
        )
    ...
    # ------------------------------------------------------------------
    # State change callback (fired from watcher threads via root.after)
    # ------------------------------------------------------------------

    def _on_state_change(self, pid: int, project: str, new_state: StatusState):
        ...
        logger.debug(
            "pid=%d project=%s new_state=%s prior_state=%s",
            pid,
            project,
            new_state.value,
            prior.value,
        ...
        # Refresh UI immediately on state change
        self._refresh_ui()
    ...
    def _refresh_ui(self):
        session_states = []
        for entry in self._sessions.values():
            # Use entry.state (set on main thread via _on_state_change), not watcher.state
            session_states.append((entry.session, entry.state, entry.container))
    ...
    def _on_row_click(self, session: SessionInfo):
        ...
        if container and container.window_handle:
            ...
            if not success:
                logger.warning(
                    "failed to foreground pid=%d hwnd=%d",
                    session.pid,
