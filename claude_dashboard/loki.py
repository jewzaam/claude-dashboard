...
"""Query Loki for the last user prompt per agent session."""
...
def _extract_prompts(results: list[dict], *, key: str, max_chars: int) -> dict[str, str]:
    ...
    for stream in sorted_results:
        ...
        if not raw_prompt and labels.get("service_name") == "codex-hook-observer":
            # Codex's hook observer keeps the complete hook payload in the log
            # body. Unlike Claude Code, it does not promote prompt to
            # structured metadata, so read it from the newest log value.
            values = stream.get("values", [])
            if values:
                try:
                    raw_prompt = json.loads(values[0][1]).get("prompt", "")
                except (IndexError, TypeError, json.JSONDecodeError):
                    logger.debug("loki Codex prompt body was not valid JSON")
        if not raw_prompt:
...
def poll_last_prompts(
    *,
    ...
    sandbox_sources: list[str] | None = None,
    ...
    ``sandbox_source`` — a sandbox's own Claude session_id is not visible
    from the host, and ``host_name`` is the machine, not the sandbox.

    Results are keyed by session_id for local sessions and sandbox_source
    for sandboxes — the caller maps these back to internal PIDs.
    """
    prompts: dict[str, str] = {}

    if session_ids:
        query = (
            '{service_name =~ "claude-code|codex-hook-observer"}'
            ' | event_name =~ "user_prompt|codex.hook.UserPromptSubmit"'
    ...
    if sandbox_sources:
        query = (
            '{service_name =~ "claude-code|codex-hook-observer"}'
            ' | event_name =~ "user_prompt|codex.hook.UserPromptSubmit"'
            f' | sandbox_source =~ "{"|".join(sandbox_sources)}"'
        )
        logger.debug("loki sandbox query: %s", query)
        try:
            results = _query_loki(loki_url=loki_url, query=query, limit=len(sandbox_sources) * 20)
            logger.debug("loki sandbox results: %d streams", len(results))
            prompts.update(_extract_prompts(results, key="sandbox_source", max_chars=max_chars))
        except Exception:
