...
- Re-verify these joins against a live stack after any stack-side label
  change: `scripts/explore/verify_otel_match.py` (both directions —
  `--local-hostname` for the hosting machine, `--no-register` for a watcher).
...
- **Headless filter reads a label already on the state metrics.** Claude Code emits no native headless marker, and the `query_source="repl_main_thread"` discriminator from 2.1.226 broke in 2.1.233 (interactive main-thread requests emit `"sdk"` too — verified live). The marker is self-made: `claude-wrapper.sh` appends `headless=true` for `-p`/`--print` runs, and the recording rules carry it through their `by()` groupings, so it rides on metrics the dashboard already polls. `_add_remote_session()` skips `state.headless`; absent label = interactive, so direct runs and old wrappers fail open. **Do not build discriminators on `query_source`** (version-unstable) and **do not resolve headless via separate Loki lookups** — the label is in the metric.
- **Remote READY renders as READY.** Missing the moment a remote session finishes is the failure that matters, so remote rows show READY like local ones. Cost, accepted: dismissal is per-dashboard local state with no telemetry to sync it, so every dashboard watching that session must dismiss it separately. Dismissal is durable — `state_cleared_from` is persisted under `host:cwd`, so a dismissed row stays IDLE across re-registration and restarts while the 30m `claude_session_ready` window keeps reporting.
...
- **Absent metrics get a grace period, not immediate removal** (`config.REMOTE_METRIC_GRACE_SECONDS`, `_expire_stale_remotes()`). The idle→working hole it was sized for is closed: `claude_session_working` is now a timestamp comparison, so `user_prompt` is itself the newest activity and the metric goes true as soon as that event lands, rather than waiting for the first `api_request` (the old hole measured median 21s, max 80s). What remains is pipeline hiccups and **mid-turn silence past the WORKING rule's range window** — a live session silent longer than that window is in none of the three metrics, because `ready` is false (activity newer than Stop) and `working`'s window is empty. **Window and grace compose:** tolerated silence is window + grace, currently 600s + 120s = 720s, so neither number can be tuned alone. 120s is at a knee, not on a slope — of the seven measured gaps over 600s, two (619.6s, 683.7s) fall under 720s and are rescued, and the next is 1211.9s, so anything from 120s to ~611s rescues the same two and buys nothing. Dismissed rows (`state_cleared_from` set) skip the grace. Sticky states and flagged rows skip expiry entirely.
