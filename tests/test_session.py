...
class TestDiscoverCodexSessions:
    """Real flock against a real /proc/locks — the join is the whole feature."""

    @staticmethod
    def _codex_home(tmp_path, *, session_id, cwd):
        home = tmp_path / ".codex"
        ...
        conn.execute(
            ...
            "created_at_ms INTEGER, source TEXT)"
        )
        conn.execute(
            "INSERT INTO threads VALUES (?, ?, ?, ?)", (session_id, cwd, 1788263941023, "cli")
        )
        conn.commit()
        conn.close()
        return home

    @pytest.mark.skipif(not config.IS_LINUX, reason="/proc/locks is Linux only")
    def test_held_lock_resolves_to_this_process(self, tmp_path):
        session_id = "01a05cd6-439f-7063-9a8f-47ace7abc7fe"
        home = self._codex_home(tmp_path, session_id=session_id, cwd="/home/user/proj")
        lock_path = home / "thread-writer-locks" / f"{session_id}.lock"
        lock_path.touch()

        with open(lock_path, "w", encoding="utf-8") as handle:
            fcntl.flock(handle, fcntl.LOCK_EX | fcntl.LOCK_NB)
            sessions = discover_codex_sessions(codex_home=home)

        assert len(sessions) == 1
        assert sessions[0].pid == os.getpid()
        assert sessions[0].session_id == session_id
        assert sessions[0].cwd == "/home/user/proj"
        assert sessions[0].started_at == 1788263941023
        assert sessions[0].harness == "codex"

    @pytest.mark.skipif(not config.IS_LINUX, reason="/proc/locks is Linux only")
    def test_stale_lock_file_without_flock_is_ignored(self, tmp_path):
        """A crash leaves the file behind; no flock means the session is gone."""
        session_id = "01a05cd6-439f-7063-9a8f-47ace7abc7fe"
        home = self._codex_home(tmp_path, session_id=session_id, cwd="/home/user/proj")
        (home / "thread-writer-locks" / f"{session_id}.lock").touch()

        assert discover_codex_sessions(codex_home=home) == []

    def test_missing_codex_home_returns_empty(self, tmp_path):
        assert discover_codex_sessions(codex_home=tmp_path / "nope") == []

    def test_returns_empty_off_linux(self, tmp_path):
        home = self._codex_home(tmp_path, session_id="x", cwd="/home/user/proj")
        with patch("claude_dashboard.session.config") as mock_config:
            mock_config.IS_LINUX = False
            assert discover_codex_sessions(codex_home=home) == []


class TestFlockOwners:
    @pytest.mark.skipif(not config.IS_LINUX, reason="/proc/locks is Linux only")
    def test_parses_blocked_waiter_lines(self, tmp_path):
        """A "->" waiter prefix shifts every column; the inode is found by shape."""
        locks = tmp_path / "locks"
        locks.write_text(
            "1: POSIX  ADVISORY  READ  111 00:19:1001 0 EOF\n"
            "1: -> POSIX  ADVISORY  WRITE 222 00:19:1002 0 EOF\n"
        )
        with patch("builtins.open", lambda *a, **k: locks.open(encoding="utf-8")):
            owners = _flock_owners()
        assert owners == {1001: 111, 1002: 222}
